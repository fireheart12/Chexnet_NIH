...
weight_others = (total_cases/others_count)/2.0
weight_nodule_mass = (total_cases/nodule_mass_count)/2.0


class_weight = {0 : weight_others , 1 : weight_nodule_mass}

checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(filepath = "new_chexnet_model.h5", 
                                                   save_best_only = True, monitor = "val_loss", mode = "min", verbose = 1)
early_stopping_cb = tf.keras.callbacks.EarlyStopping(patience = 10, restore_best_weights = True, monitor = "val_loss", mode = "min", 
                                                     verbose =1)
lr_reducer_cb = tf.keras.callbacks.ReduceLROnPlateau(monitor = "val_loss", factor = 0.1, patience = 5, min_lr = 1e-7, mode = "min", verbose = 1)
...
with tf.device("/device:GPU:0"):
    ...
    inp = tf.keras.Input(shape = (config["IMAGE_SIZE"][0], config["IMAGE_SIZE"][1], 3))
    base_model = tf.keras.applications.DenseNet121(weights = "imagenet", include_top = False, 
                                                   input_shape = (config["IMAGE_SIZE"][0], config["IMAGE_SIZE"][1], 3), pooling = "avg")
    x = base_model(inp)
    x = tf.keras.layers.Dropout(0.5)(x)
    output = tf.keras.layers.Dense(units = 1, activation = "sigmoid", name = "predictions")(x)
    
    model = tf.keras.Model(inp, output)
    model.compile(tf.keras.optimizers.Adam(lr = 0.0001), loss = "binary_crossentropy", metrics = [tf.keras.metrics.AUC(multi_label = False)])
    model.summary()
